...
def nothing(x):
    ...
    #saturation값과 value값을 설정을 위한 Trackbar을 가져온다.
...
while(True):
    ...
    if set_color == False:
        #색상을 타겟팅하기 위해 녹색 사각형을 화면에 나타낸다.
        rectangle_color = (0, 255, 0)

        #색을 받았다면 step 1이 되어 한번더 타게팅 할 수 있다.(2번째는 빨간색 사각형이다.)
        if step == 1:
    ...
    #step == 0 은 첫번째 선택하는 색상.
        ...
        if step == 0:
            ...
            cv.imshow("roi1", roi)
            #hsv 색공간으로 변환하여 색 추출을 한다.
            hsv = cv.cvtColor(roi, cv.COLOR_BGR2HSV)
            h,s,v = cv.split(hsv)
            color1 = h.mean()
            print(color1)
            step += 1
        #step ==1 은 두번째 선택받는 색상이다.
        elif step == 1:
            roi = img_color2[cy-20:cy+20, cx-20:cx+20]
            roi = cv.medianBlur(roi, 3)
            cv.imshow("roi2", roi)
